...
public class AccountApp {
	...
	private static void executeOperation(int choice) throws IOException {
		switch (choice) {
		case 1:
			addAccounts();
			break;
...
//			showAccounts();
	...
	private static void exportAccounts() throws IOException {
		String filePath = AccountFiles.enterFilePath();
		String fileName ;
		if (filePath != null) {
			System.out.println("VALID FILE PATH ==>"+ filePath);
			fileName = AccountFiles.enterFileName(filePath);
			AccountFiles.writeFileAccounts(filePath, fileName, accounts);
		}
	}

	private static void importAccounts() throws FileNotFoundException {
		// 1. enter file location - path
		// 2. enter file name
		// 3. read file and store records in accounts data structure (array)
		...
		if (filePath != null) {
			System.out.println("VALID FILE PATH ==>"+ filePath);
			fileName = AccountFiles.enterFileName(filePath);
			// initAccounts data structure
			accounts = AccountFiles.readFileAccounts(filePath, fileName);
			printAllAccountsIds();
		}
		
//		String fileName = AccountFile.enterFileName();
