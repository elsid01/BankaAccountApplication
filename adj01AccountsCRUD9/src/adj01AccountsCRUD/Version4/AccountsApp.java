...
public class AccountsApp {
	...
	private static void executeOperation(int choice) throws FileNotFoundException {
		switch (choice) {
		case 1:
			addAccounts();
			break;
...
//			exportAccounts();
	...
	private static void importAccounts() throws FileNotFoundException {
		...
		if (filePath != null) {
			...
			fileName = AccountFile.enterFileName(filePath);
			// initAccounts data structure
			accounts = AccountFile.readFileAccounts(filePath, fileName);
			printAllAccountsIds();
		}
